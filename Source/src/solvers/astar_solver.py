...
def compute_heuristic(cnf: CNF, assignment, var_index_map):
    h = 0
...
def clause_status(clause: list[int], assignment, var_index_map):
    unassigned = []
    for lit in clause:
        var = abs(lit)
        idx = var_index_map[var]
...
def unit_propagate(assignment, cnf: CNF, var_index_map):
    changed = True
    while changed:
        changed = False
        for clause in cnf.clauses:
...
def check_full_assignment(cnf: CNF, assignment, var_index_map):
    for clause in cnf.clauses:
        clause_satisfied = False
        for lit in clause:
            var = abs(lit)
...
def expand_state(cnf: CNF, assignment, level, variables, var_index_map, var_to_clauses):
    next_states = []
...
def solve_with_astar(grid: Grid):
    try:
        ...
        initial_f = initial_level + compute_heuristic(
            cnf, initial_assignment, var_index_map
        )
        open_list = []
        heapq.heappush(
            open_list, (initial_f, initial_level, counter, initial_assignment)
        )
        nodes_expanded = 0
        ...
        while open_list:
            ...
            if is_complete_assignment(assignment):
                if check_full_assignment(cnf, assignment, var_index_map):
                    ...
                    # )
                    model = [
                        var if assignment[var_index_map[var]] else -var
                    ...
                    if validate_solution(islands, edge_vars, model):
                        ...
                        if check_hashi(islands, hashi_solution):
                            ...
                            # print(
                            #     "Warning: Solution failed final check_hashi despite passing CNF validation."
                            # )
                            pass
                continue

            for new_level, new_assignment in expand_state(
                cnf, assignment, level, variables, var_index_map, var_to_clauses
            ):
